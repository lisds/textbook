...
## Rules to avoid trouble for Pandas < 1.5

The rest of this page has some explanation of the problems that can come up for
older Pandas, or when you do not enable the `mode.copy_on_write` option. If you
are in either of those situations, this page gives some rules that we suggest
you follow to stay out of trouble.
...
**Note**: This question only arises, in practice, when you have not set the
`mode:copy_on_write` option to `True`.  If you have set that option, then the
answer to the question below is that (effectively) `clarity` is always a copy.
...
modify the data in the view, that means we also modify the original DataFrame,
...
We strongly suggest that when you get stuff out of a Pandas DataFrame or
...
No, we didn't change the original DataFrame — and we conclude that `row_0` is a
*copy*.
