...
We start by making the DataFrame with the four possible label pairs.
...
```{python}
second_label_pair = ['No', 'rescuer']
third_label_pair = ['Yes', 'bystander']
fourth_label_pair = ['Yes', 'rescuer']
fourth_label_pair
```

We can put these label pairs into their own list.  This is a list of lists,
where the list elements are themselves lists.
...
If you repeat a row label, you will get the same row repeated.  Here is how we
get three repeats of the first row:
...
label row 64 times, then the 'No', 'rescuer' label row 165 times, and so on.

To find the number of repeats for each row, we fetch the rows from the
original cross-tabulation.  The 'No' row gives the 'bystander' and 'rescuer'
repeat numbers for the 'No' respondents

```{python}
# bystander, rescuer counts for the No respondents
no_counts = bystander_tab.loc['No']
no_counts
```

Similarly we can get the 'Yes' counts for 'bystander' and 'rescuer's.

```{python}
yes_counts = bystander_tab.loc['Yes']
yes_counts
```

We use `np.concatenate` to concatenate the 'bystander' and 'rescuer' label counts for
'No' and those for 'No'.
...
#   Yes, bystander
#   Yes, rescuer
both_rows = np.concatenate([bystander_tab.loc['No'], bystander_tab.loc['Yes']])
both_rows
```

We want to repeat the first row (label 0) of the pairs data frame 64 times, the
second (label 1) 165 times, and so on:

```{python}
# The 280 labels we want to fetch from the pairs data frame.
labels_repeated = np.repeat([0, 1, 2, 3], both_rows)
labels_repeated
```
...
As you can see, and as you might have predicted, we now have 64 rows with label
0, 165 rows with label 1, and so on.  To clean this up, and give each row its
own unique label, we can reset the index, throwing away the old index with the
`drop=True` argument:
...
Next, shuffle the `party_yn` values, and put them back into a fake version of
the Entries Table data frame:
