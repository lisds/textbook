...
We can get our observation table column labels from the counts table as well:

```{python}
obs_cols = [fishers_counts.index.name, fishers_counts.columns.name]
obs_cols
```
...
these_obs[obs_cols[0]] = np.repeat([row_label], n)
these_obs[obs_cols[1]] = np.repeat([col_label], n)
these_obs
```
...
def counts2obs(counts_tab):
    obs_cols = [counts_tab.index.name, counts_tab.columns.name]
    obs_rows = []  # List to collect observation rows.
    for row_label in row_labels:
        for col_label in col_labels:
