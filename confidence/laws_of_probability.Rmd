...
Of the 30 (30%) of red cars, 20% are Toyotas.

Think of this probability as the probability that a car is Toyota, given that is red, it 0.2:
...
For example, imagine that it's much more common cars that aren't red to be
Toyotas (red is an unpopular color for Toyatas, say).

We find that, of the red cars, 20% are Toyotas.  But it might then be true,
that of *all* cars, maybe 50% are Toyotas (remember, this car park is popular
with taxis).
...
A common mistake is to find this figure (here $P(\text{Toyota} = 0.5$ is the
probability we want — when we actually need the *conditional* probability
$P(\text{Toyota} | R)$.

We can only use this *unconditional* probability when it the same as the conditional probability, and this is only true when — in our case — the red and the not-red cars have exactly the same chance of being Toyota.  Put mathematically:
...
In this case — where the conditional probability is the same for both red and
not red — we say that the chance of getting a Toyota is *independent* of the
color.  And that is not true in our case.
