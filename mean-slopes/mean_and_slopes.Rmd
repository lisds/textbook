...
calc_sses = np.zeros(n_slopes)
# Also, get ready to record slope with lowest value.
best_slope = None
least_error = np.inf
...
```{python}
plt.plot(some_slopes, calc_sses)
...
```{python}
print('Smallest error', np.min(calc_sses))
print('Smallest error we found in the loop', least_error)
```

Here is the slope value that corresponds the minimum sum of
squared error:
