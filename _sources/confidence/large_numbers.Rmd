...
Here is our record of all the cars in the car park:

```{python}
n_in_mall = 20_000
n_red_cars = round(p_red * n_in_mall)
west_mall = np.repeat(['red', 'not_red'], [n_red_cars, n_in_mall - n_red_cars])
```

Here we think about what happens as we take samples with larger and larger
number of cars, and take the proportion (this is also an average) of not_reds in
the sample.

The law of large numbers says that, as we take more samples of the cars, the proportion of not_reds in the sample will get closer and closer to the probability of getting not_red on any one trial.
...
```{python}
cars = rng.choice(west_mall, size=100)
prop = np.count_nonzero(cars == 'red') / 100
print('Proportion of not_red', prop)
```

Now we run 1000 trials.  On each trial we get 100 cars, and record the
proportion of not_red.  Finally we show the spread of proportions that we get.
...
for i in np.arange(n_iters):
    samples = rng.choice(west_mall, size=n_cars)
...
sample gets very high, the proportion of not_red in the sample gets very close
to 0.3.
...
will flow along the not_red path, and a probability of 0.7 that it will flow
along the red path. *In the long* run, 30% of the cars end up at the end of the not_red path, and 70% of trials end up at the end of the red path.
