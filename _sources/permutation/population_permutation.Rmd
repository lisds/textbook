...
<../data/brexit_survey.csv>` to the same directory as this notebook.
...
First we make two new DataFrames, one for the Leavers, and the other for the
Remainers:
...
print('Number of Remainers:', n_remain)
leavers = brexit_df[brexit_df['brexit_vote'] == 'Leave']
# Number of Leavers.
n_leave = len(leavers)
print('Number of Leavers:', n_leave)
# Show some remainers
remainers.head()
```

We get the ages for the Leavers and the Remainers.

```{python}
# Get data frames for leavers and remainers
remain_ages = remainers['age']
leave_ages = leavers['age']
```
