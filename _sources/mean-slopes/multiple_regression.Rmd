...
We looked at simple regression the [finding
...
This page is about *multiple regression*.  Multiple regression take simple
...
* `White Blood Cell Count`: white cells are the immune cells in the blood.
  ...
  about whether the white blood cell count is an useful measure in chronic
  kidney disease.
...
The other way to approach this problem is *multiple regression*.  In multiple regression, we use multiple columns of data to predict our measure of interest — in this case — the `Creatinine` values.
...
In multiple regression, we have more than one column of predicting values.  For each, we calculate a matching slope.  In our new case here, the two columns of predicting values are `Urea` and `Hemoglobin`.  We therefore have to find:
...
For the simple case, predicting `Creatinine` from `Urea`, we got the predicted
values by multiplying the slope for `Urea` (`s`) by the `Urea` values, and
adding the intercept (`c`).  The slope for Urea `s` gives the *scaled amount of
Urea to add to the prediction*. So the calculation is:

```{python}
simple_predictions = c + s * ckdp['Urea']
```

When we have two predictors, Urea and Hemoglobin, we multiply the slope for
Urea by the Urea, and the slope for Hemoglobin by the Hemoglobin, and add both
of these to the intercept.  The prediction is:
...
We repeat the RMSE calculation above, using the function:
...
```{python}
params = multiple_regression_triple(x1_values, x2_values, y_values)
params
```
...
At the top of this pages, we started with finding two parameters:
...
* the intercept plus
* the slpes for Urea times the Urea values plus
* the slpes for Hemoglobin times the Urea values.

In fact we can extend this idea futher by adding more columns of values, and more slopes.  For example
...
def rmse_n_params(params, attributes, y_values):
    ...
    for col_no in np.arange(len(slopes)):
        col = attributes.iloc[:, col_no]
        col_contribution = slopes[col_no] * col
        predictions = predictions + col_contribution
    errors = y_values - predictions
    return np.sqrt(np.mean(errors ** 2))
```
...
attributes = ckdp.loc[:, ['Urea', 'Hemoglobin']]
# Recalculate the RMSE
rmse_n_params([1, 0, 0], attributes, ckdp['Creatinine'])
```

```{python}
min_css = minimize(rmse_n_params, [1, 0, 0], method='powell',
                   args=(attributes, ckdp['Creatinine']))
min_css
```
...
def multiple_regression_matrix(attributes, y_values):
    ...
    attributes: array-like, shape (n, p)
        2- dimensional array where each column contains
    y_values : array-like, shape (n,)
        Values to be predicted.
