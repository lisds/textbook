...
```{python}
best_c, best_s = minimize(rmse_with_unpacking, [2.25, 0.47]).x
print(best_c)
print(best_s)
```
...
```{python}
def rmse_with_extra_numbers(c_s, p, q):
...
minimize(rmse_with_extra_numbers, [2.25, 0.47], args=(99, 101))
```

Notice that, each time `minimize` calls `rmse_with_extra_numbers` it passes the
first value of `args` as the second argument to the function (`p`) and the
second value of `args` as the third argument to the function (`q`).
...
```{python}
minimize(rmse_clarity, [2.25, 0.47])
```
