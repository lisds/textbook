...
# Prepare to record slope with lowest value.
best_slope_for_2p25 = None
least_error = np.inf
for i in np.arange(n_slopes):
    slope = some_slopes[i]
    this_error = sos_error_c_s(2.25, slope)
    # Record this slope if this is the smallest error.
    if this_error < least_error:
        least_error = this_error
        best_slope_for_2p25 = slope
    # Record the error measure in error array.
    sos_errors[i] = this_error
```
...
def best_slope_for_intercept(intercept, some_slopes):
    ...
    for i in np.arange(n_slopes):
        slope = some_slopes[i]
        this_error = sos_error_c_s(intercept, slope)
        # Record this slope if this is the smallest error.
        if this_error < least_error:
            least_error = this_error
            best_slope = slope
    ...
    return np.array([best_slope, least_error])
```
...
for i in np.arange(n_intercepts):
    ...
    slope_and_error = best_slope_for_intercept(intercept, some_slopes)
    this_best_slope = slope_and_error[0]
    this_error = slope_and_error[1]
    if this_error < least_error:
        least_error = this_error
        best_intercept = intercept
        best_slope = this_best_slope
    # Store the slope and error in their arrays.
    best_slopes[i] = this_best_slope
    lowest_errors[i] = this_error
...
print('Least error', min(lowest_errors))
print('Least error as found in loop', least_error)
```
...
We have already recorded the corresponding intercept and slope.
