...
We imagine an ideal world where rescuers and bystanders have exactly the same
tendency to belong to a political party.
...
The `people` DataFrame would have 64 rows with 'No' in — say — a `party_yn`
column and a 'bystander' label in — say — a `respondent` column, followed by 7
rows with 'Yes' in `party_yn` and 'bystander' in `respondent`, and so on.

We start by making a DataFrame that contains all four possible label pairs.
...
Here are the remaining three label pairs:

```{python}
second_label_pair = ['Yes', 'bystander']
third_label_pair = ['No', 'rescuer']
fourth_label_pair = ['Yes', 'rescuer']
fourth_label_pair
```

We can put these label pairs into their own list.  This is a list of lists, where the list elements are themselves lists.

```{python}
# A list of lists.
all_pairs = [first_label_pair, second_label_pair, third_label_pair, fourth_label_pair]
all_pairs
```

For example, the first element in this list is the `first_label_pair` list.

```{python}
all_pairs[0]
```

We can use `pd.DataFrame` to make a new DataFrame from this list of lists.
Each list in `all_pairs` will be a row in the DataFrame.

```{python}
label_pairs = pd.DataFrame(all_pairs)
label_pairs
```
...
```{python}
label_pairs = pd.DataFrame(all_pairs, columns=['party_yn', 'respondent'])
label_pairs
```

As you know, you can get the row with the label 0, using `.loc` indexing.

```{python}
# Get the row with label 0.
label_pairs.loc[0]
```

You can get more than one row with `.loc` by passing a list with more than one
label:
...
If you repeat a row label, you will get the same row repeated:

```{python}
# Repeating a row.
label_pairs.loc[[0, 0, 0]]
```

Leaping ahead, we are going to use this trick to repeat the 'No', 'bystander'
label row 64 times, then the 'Yes', 'bystander' label row 7 times, and so on.

To find the number of repeats for each row, we fetch the columns from the
original cross-tabulation.  The 'bystander' column gives the 'No' and 'Yes'
repeat numbers for the columns labeled 'bystander'.

```{python}
# Yes, No label counts for bystanders
bystanders = bystander_tab['bystander']
bystanders
```

Similarly we can get the 'rescuer' 'No' and 'Yes' numbers from the 'rescuer' column.

We use `np.concatenate` to concatenate the 'No' and 'Yes' labels counts for
'bystander' and those for 'rescuer'.
...
#   No, rescuer
#   Yes, rescuer
both_cols = np.concatenate([bystander_tab['bystander'], bystander_tab['rescuer']])
both_cols
```

We want to repeat the first row (label 0) of the pairs data frame 64 times, the
second (label 1) 7 times, and so on:

```{python}
# The 280 labels we want to fetch from the pairs data frame.
labels_repeated = np.repeat([0, 1, 2, 3], both_cols)
labels_repeated
```

We now have one value per row in our eventual `people` DataFrame:

```{python}
len(labels_repeated)
```

We use the repeated labels to duplicate the pairs the given number of times, in
order to form that output `people` DataFrame:

```{python}
# Make the rows for "party_yn" and "bystander" by repeating rows from pairs.
people = label_pairs.loc[labels_repeated]
people
```

As you can see, and as you might have predicted, we now have 64 rows with label 0, 7 rows with label 1, and so on.  To clean this up, and give each row its own unique label, we can reset the index, throwing away the old index with the `drop=True` argument:

```{python}
# Labels are annoying, reset, dropping the old index.
...
# Make a copy to make absolutely sure we not changing the original.
# (In fact mode.copy_on_write would take care of this for us, but
# the explicit copy makes our intention clear).
...
chi2_val, p_val, degf, expected = sps.chi2_contingency(
    people_tab,
    correction=False)
# Chi2 test returns a two-tailed p-value.
# One tailed is the two-tailed value / 2.
p_val / 2
```
