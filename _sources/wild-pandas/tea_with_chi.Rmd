...
```{python}
plt.hist(fake_yes_yes, bins=np.arange(5));
```
...
The *expected* value is the average value *in the long run* - if we do an infinite number of trials.  

**Note** - this is actually a technical term in statistics.  The *expectation* of a particular value is the value of the mean in the long run.
...
stat, p_value, dof, exp_array = sps.chi2_contingency(observed_counts,
                                                     correction=False)
exp_array
```

We can then look at how much the expected counts differ from the actual counts we saw in the real world.  We could go through subtracting the entries in the expected counts table from the entries in the observed counts table, one by one, but Pandas is clever enough to understand this is what we want when we subtract the two DataFrames directly:
...
In textebooks, you will often see the observed values written as "O" (capital "o") and the expected values written as "E":
...
The observed minus expected table shows us how far each entry in the table was from the expected value.  Call this the *deviations* table.


We would like an overall measure of how far the observed table was from the expected table.   We could just add up all the values in the deviation table, like this:
...
We can now take an overall measure of how far the table is from expected, by summing up the values in this proportional squared deviaton table, to get the *chi-squared statistic*:
...
This is exactly the statistic that the Scipy chi-squared routine calculates for us, along with the expected table:
...
It turns out that we can also calculate a probability of seeing a chi-squared value as large or larger, on the assumption that the relationship between the cross-tabulated labels is actually random.  The details of this involve a lots more mathematics than we can go into in this course.

```{python}
p_value
```

As you can see, this probability value is a lot smaller than the one we calculated before, and that is because we have unwisely turned of the necessary correction for 2 by 2 tables in the Scipy routine:

```{python}
# chi-squared calculation with correction for 2x2 tables
stat, p_value, dof, exp_array = sps.chi2_contingency(observed_counts,
                                                     correction=True)
p_value
```
...
But - what if there are not the same number of labels in the two categories?  What are the expected values in that case.
...
```{python}
n_iters = 1000
fake_yes_yes_2_6 = np.zeros(n_iters)
for i in np.arange(n_iters):
...
```{python}
plt.hist(fake_yes_yes_2_6, bins=np.arange(4))
```

We see that, in the null-world, the *average* count for the `yes`, `yes` entry is around 0.5:
...
```{python}
O = observed_counts_2_6
O
```

There must always be 2 `yes` labels in this row (2 cups with milk-first) to distribute between the 2 rows (Muriel says `no`, Muriel says `yes`):

```{python}
row_totals = O.sum(axis='columns')
row_totals
```
...
```{python}
E = pd.DataFrame([[6 * 6 / 8, 6 * 2 / 8], [2 * 6 / 8, 2 * 2 / 8]],
                 index=['no', 'yes'],
                 columns=['no', 'yes'])
E
```
...
```{python}
stat, p_value, dof, exp_array = sps.chi2_contingency(O, 
                                                     correction=False)
```
...
## Randomization and Chi-squared


**Summary**:

* Randomization - choose your own statistic, randomize, p-value by comparing to the sampling distribution.  Common statistics are particular values from the observed counts table, or chi-squared.
* Chi-squared - a statistic that applies to the whole table.  Uses some mathematics to calculate the p-value.


In the old days, before it was common to use randomization, you would need to know some rules about when to use particular tests.  The rules were something like:
...
Now you have resampling, you can just choose your statistic and calculate your sampling distribution, in order to get your p-value.
