...
In this case the histogram is a little messy.  We will use a simple routine that counts the number of 0s, 1s, 2s ... and then plots those counts as a bar graph.
...
As you can see, this probability value is a lot smaller than the one we calculated before, and that is because we have unwisely turned off the necessary correction for 2 by 2 tables in the Scipy routine:
...
```{python}
stat, p_value, dof, exp_array = sps.chi2_contingency(
