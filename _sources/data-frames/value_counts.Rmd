...
We might want to know how many people boarded at each of these ports.  For example, we might want to know how many people joined the Titanic at Belfast.   We could do this with [row indexing](df_basic_row_indexing), but applied to the Series.

It works like this:

* First make a Boolean Series with True where `embarked` == `'Belfast`
* Use this Boolean series to index into the `embarked` Series, and select the
  values corresponding to Belfast.

Finally, we can count how many values remain, to give a count of the number of rows (people) who embarked at Belfast.
...
```{python}
# Select values corresponding to Beflast, by indexing.
belfast_values = embarked[is_belfast]
belfast_values
```
