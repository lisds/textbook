...
You can see the row labels on the left hand side of the rows in the default display:

```{python}
gender_data
```

Notice that the label of the first row is 0, the label of the second row is 1,
and so on.
...
Notice that Pandas stores the row labels in an `Index` object (value), just as
it stored the column labels in an `Index` object (value).
...
Notice that we have now dropped the row with label 0, and the row with label 2,
and the first row (position 0) now has label 1.  We have dropped the row with
label 2, so the second row (position 1) now has label 3.  The row labels do not correspond to the row positions any more.


## Using different row labels

As we said above, the row labels are numbers, but default, but they need not be numbers — they could be strings.

Sometimes it is useful to change the default numeric row labels to something else that is a better name for the row.
...
codes, instead of the numeric labels we had before.  Notice too that Pandas pulled the `country_code` column out of the DataFrame, to avoid duplication.  You can tell Pandas not to do pull the column out of the DataFrame, by adding `drop=False` to the argument for `set_index`.
