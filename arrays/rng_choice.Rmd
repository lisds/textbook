...
Set our potential choices:

```{python}
juror_types = ['Black', 'White']
```

Now we want to select `"Black"` with probability 0.26 (26%) and `"White"` with
...
rng.choice(juror_types, p=[0.26, 0.74])
```

Run the cell above a few times to persuade yourself that you see "Black" about
quarter of the time, and "White" otherwise.

The `choice` method has another useful keyword argument, `size`.  We can use this to ask `choice` to make multiple choices at the same time.  `choice` returns the choices as an array.  For example, say we wanted to choose 12 jurors, each with a probability of 0.26 of being `"Black"`.  We could do this:

```{python}
# Select 12 jurors in one go.
rng.choice(juror_types, p=[0.26, 0.74], size=12)
```

Now consider the problem of choosing 4 children, each with a probability of 0.5
of being a Girl.  Let's do the same as we did before, and use 0 for a boy, and 1 for girl.
