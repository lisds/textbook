...
Here's are the GDP values as a Series.  We are using *direct indexing with a column label* on the DataFrame, to get the column as a Series:

```{python}
# Direct indexing with column label.
gdp = labeled_gdata['gdp']
gdp
```
