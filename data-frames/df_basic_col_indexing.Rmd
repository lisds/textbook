...
# Indexing into DataFrames to select columns
...
We all also call this *direct indexing with a column label*.
...
What `type` of value came back from this *direct indexing with a column label*?
...
each value now has it's corresponding row label.  The index values are on the left.
...
To go forther, we will see what we can do by sorting the DataFrame, and looking at the first and last rows.
